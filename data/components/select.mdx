...
- `setHighlightedOption(...)` — Function to set the highlighted option.

```jsx
api.isOpen // => boolean

api.highlightedOption // => { label: string, value: string } | null

api.selectedOption // => { label: string, value: string } | null

api.blur() // => void

api.focus() // => void

api.setSelectionOption({ label: string, value: string }) // => void

api.setHighlightedOption({ label: string, value: string }) // => void
```
