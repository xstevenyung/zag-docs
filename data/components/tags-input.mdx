...
api.setValue(["item-2", "item-3"]) // => ["item-2", "item-3"]

api.valueAsString // => "['item-2', 'item-3']"

api.addValue("item-4") // => ["item-2", "item-3", "item-4"]

api.deleteValue("item-2") // => ["item-3", "item-4"]
...
When an option is disabled, the `data-disabled` attribute is set on the option.

```css
[data-part="option"][data-disabled] {
  /* styles for disabled option  */
}
```

### Selected state

When an option is selected via the keyboard navigation or pointer hover, a
`data-selected` attribute is set on the option.
