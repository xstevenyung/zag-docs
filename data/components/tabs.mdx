...
- `setValue()` — Used to set the value of the tab that will be shown.
...
When any tab is focused, the trigger group is given a `data-focus` attribute.

```css
[data-part="trigger-group"][data-focus] {
...
[data-part="trigger-group"][data-orientation="(horizontal|vertical)"] {
  /* Styles for horizontal/vertical tab-indicator */
}
```
